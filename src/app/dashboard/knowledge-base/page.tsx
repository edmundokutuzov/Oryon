...
export default function KnowledgeBasePage() {
  return (
    <div className="p-6 fade-in">
        ...
        <p className="text-white/60 mb-8">Encontre guias, políticas e artigos úteis da empresa.</p>
        <div className="relative mb-6">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 w-5 h-5" />
            <Input type="text" placeholder="Pesquisar artigos..." className="w-full pl-12 pr-4 py-3 h-auto bg-white/10 border-white/20 rounded-xl focus:outline-none focus:border-primary text-white" />
        </div>
            ...
            {knowledgeBase.map(article => (
                <Card key={article.id} className="gradient-surface border-0 rounded-2xl hover:shadow-xl transition-shadow cursor-pointer p-6">
                   <CardHeader className="p-0">
                     ...
                     <CardTitle className="text-lg font-bold text-white mb-2">{article.title}</CardTitle>
                   </CardHeader>
                    <CardContent className="p-0">
                        <div className="flex items-center text-xs text-white/60 mt-4 space-x-4">
